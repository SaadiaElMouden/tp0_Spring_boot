    ...
    <tbody>
        <c:forEach var="product" items="${products}">
            <tr>
                ...
                <td>
                    <a href="<%= request.getContextPath() %>/product-form?action=edit&id=${product.id}" class="btn btn-primary btn-sm">Modifier</a>
                    <a href="<%= request.getContextPath() %>/delete-product?id=${product.id}" class="btn btn-danger btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce produit?')">Supprimer</a>
                </td>
            </tr>
        </c:forEach>
    </tbody>
